create function checkconcerttimeduplication(starttime timestamp without time zone, endtime timestamp without time zone) returns SETOF business_service_schema.concerts
    language plpgsql
as
$$
BEGIN
    return query
        select concerts.*
        from business_service_schema.concerts
                 join business_service_schema.show_times on concerts.id = show_times."concertId"
...
BEGIN
    return (SELECT COUNT(DISTINCT concerts.id)
            FROM business_service_schema.concerts
                     LEFT JOIN business_service_schema.show_times st ON concerts.id = st."concertId"
...
BEGIN
    RETURN QUERY
        ...
        FROM business_service_schema.concerts
                 LEFT JOIN business_service_schema.show_times st ON concerts.id = st."concertId"
                 LEFT JOIN business_service_schema.images i on concerts.id = i."concertId"
        WHERE "startTime" BETWEEN NOW() AND NOW() + INTERVAL '14 days'
